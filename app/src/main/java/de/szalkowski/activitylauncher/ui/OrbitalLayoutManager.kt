...
class OrbitalLayoutManager(context: Context) : RecyclerView.LayoutManager() {
    ...
    private val radius = (160 * context.resources.displayMetrics.density)

    init { isItemPrefetchEnabled = true }

    override fun generateDefaultLayoutParams(): RecyclerView.LayoutParams =
        RecyclerView.LayoutParams(132, 156)
    ...
    override fun onLayoutChildren(recycler: RecyclerView.Recycler, state: RecyclerView.State) {
        ...
        for (slot in 0 until visible) {
            ...
            val x = centerX + (cos(angle) * radius.toDouble()).toInt() - getDecoratedMeasuredWidth(child) / 2
            val y = centerY + (sin(angle) * radius.toDouble() * 0.38).toInt() - getDecoratedMeasuredHeight(child) / 2
            layoutDecoratedWithMargins(child, x, y, x + getDecoratedMeasuredWidth(child), y + getDecoratedMeasuredHeight(child))
